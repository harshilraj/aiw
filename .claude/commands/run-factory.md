...
Gate: `factory.tailored` must be true.

Soft check: `agency.brandLocked` should be true. If it is false, warn the student that Stage 13 (proposal builder) will halt mid-pipeline because `website-factory/clients/_agency/agency-brand.json` is missing its required values. Recommend they run `/setup-agency` first, but let them proceed if they confirm they want to (some students prefer to ship the site build first and add the proposal layer later).
