...
7. Generates the `PAGE_DATA` `<script>` object from our the website template route list + section composition
8. Copies `[X] Website/dist/` → `[X] Proposal/build/` so the iframe at `../build/index.html` resolves
9. Validates: zero `{{VAR}}` leakage, all per-lead assets present, the website template build has `#hero`/`#about`/`#service-area` IDs
...
| Build anchor IDs missing (`#hero` / `#about` / `#service-area`) | default template's section IDs drifted. Confirm `templates/website-template/src/components/Hero.jsx` carries `id="hero"` on its `<section>`, `Founder.jsx` carries `id="about"`, `ServiceAreas.jsx` carries `id="service-area"`. Re-run Stage 10.1 to rebuild. NB: validator greps the bundled JS at `dist/assets/index-*.js`, not `dist/index.html`, the SPA shell never contains these IDs (only the React-mounted runtime DOM does). |
...
| Build IDs missing | Halt with `failed`, surface which IDs are missing in `dist/assets/index-*.js`, fix the website template components (Hero/Founder/ServiceAreas) + re-run Stage 10.1 |
