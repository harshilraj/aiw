...
then feeds that DNA into the brand board generation step (Stage 8) and the build (Stage
10.1).
...
- **Motif detection**: visual inspection. Identify any recurring graphic mark (lightning
  bolt, mountain, shield, sun, etc.). Record description in one sentence for brand-board
  prompt injection.
...
back to defaults silently, the schema is the contract the brand-board agent and build
agent depend on.
