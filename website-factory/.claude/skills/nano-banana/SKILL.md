...
| Universal prompt scaffold | `templates/nano-banana-hero-prompt.template.md` |
| Niche composition fragment | `templates/{active-niche}/niche-playbook/hero-composition.md` |
...
4. **Prompt assembly.** Fill `templates/nano-banana-hero-prompt.template.md` with: company
   name, city/state, region defaults, brand-dna.palette, services, mood lighting brief
   (from niche mood map), niche composition fragment, and any reference-photo blocks.
...
| Logo unrecognisable in output | Note in metadata, do not halt (manual review at `/approve-brand-board`) |
...
python3 tools/generate-hero.py --client "[Client Name]" --no-subject
```
