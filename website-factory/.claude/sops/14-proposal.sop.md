...
| §10 | Section 10 · What's Included (Sitemap Tree) | Relume-style tree → click any of 12 page boxes → `<dialog id="pageDialog">` opens, populated by `PAGE_DATA` JS map (17 pages × 8-14 sections each). | seo/sitemap-plan.md + JS PAGE_DATA |
...
1. **`text-wrap: balance`** redistributes words across lines so the visual mass is balanced. Without it, browsers greedy-wrap and orphan single words on a line (e.g. `The the website template's hero / Website / for Gravity Roofing.`, "Website" stranded on line 2).
...
- H1: 3-line stacked title with explicit `<br>` breaks (2026-05-11 typography overhaul):
  ...
  ```
  - Default h1 weight: **500 (medium)**, `the website template's hero` reads light
...
- **Shared `<dialog id="pageDialog">`** populated by JS `openPage(id)` from a `PAGE_DATA` object map (defined in script before `</body>`). Each page has 8-14 sections rendered as `.pc-section-row` (gold-numbered badge + title + desc + category tag). Modal styled white-bg with `.pc-modal-head` + `.pc-modal-body` (light grey bg `#F7F5F0`).
...
- **2026-05-06 (late)**, Major conversion-focused upgrade across 7 sections:
  ...
  - **§6 What's Included**: rebuilt as **Relume-style white-mode tree** (`#F7F5F0` panel bg). Home root + Tier 1 Core + Tier 2 Pillars + Tier 3 Silos. Connector rail + drops sit OUTSIDE cards (verified 2px gap above card top). All 12 main cards (root + 5 Core + 7 Pillars) clickable, opening a shared `<dialog id="pageDialog">` populated by JS `PAGE_DATA` map (8-14 sections per page in Relume page-card layout). Replaces the old flat ordered-list sitemap.
