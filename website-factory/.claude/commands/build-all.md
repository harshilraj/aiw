...
| Sequential | 6 Copy deck | `05-copy-deck.md` | copy-deck.md written, validator confirms zero the website template string leakage |
...
| Sequential | 10.1 Build | `09-build.md` | `tools/build-from-template.py --client "[X]"` clones templates/website-template, overlays brand-dna.js, copies + optimises assets, npm install + npm run build, validator passes (no `__REQUIRED__` sentinels, no forbidden strings in dist) |
| Sequential | 10.2 Personalize | `10-personalize.md` | sitemap.xml + schema injected |
| Sequential | 10.3 Uplift | `11-uplift.md` | optional extras (scoped down vs prior approach; most things already in the website template) |
| Sequential | 10.4a Design fidelity | `design-fidelity-qa-agent.md` | SSIM 100% structural match against templated reference (build templates/website-template with this client's brand-dna applied to a temp dir) |
| Sequential | 10.4b SOP QA | `sop-qa-agent.md` | BOTH website template composition AND universal SOP at 100%, OR `/override-sop` |
| Sequential | 10.4c Build fidelity | `10-4c-build-fidelity.md` | `tools/build-fidelity-diff.py` reports zero structural mismatches (DOM tree shape matches templates/website-template), 100% required |
