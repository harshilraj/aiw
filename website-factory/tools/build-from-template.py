...
per-client assets, runs `npm install && npm run build`, then validates the
dist for sentinels and forbidden strings.

Usage:
    python3 tools/build-from-template.py --client "Acme Roofing"
    python3 tools/build-from-template.py --client "Acme Roofing" --skip-install   # iterate fast
    python3 tools/build-from-template.py --client "Acme Roofing" --dry-run        # compose only, no build
...
Reads (per client):
    ...
    clients/[X]/Pipeline Data/brand-resonance/resonance.json     (optional, Stage 7.5)
...
Writes:
    clients/[X]/[X] Website/                              (full the website template clone)
...
Validator fails closed on:
    - any __REQUIRED__ sentinel surviving in src/config/brand-dna.js
    - any FORBIDDEN_STRINGS entry (__REQUIRED__ sentinel + niche-playbook extensions)
      in dist/index.html or dist/assets/*.css/*.js
"""
...
DEFAULT_TEMPLATE_DIR = TEMPLATES_DIR / "website-template"
...
def resolve_active_template(cli_niche: str | None) -> tuple[Path, str | None]:
    """Pick the source template directory for this build.

    Resolution order:
      1. --niche CLI argument (explicit override)
      2. Stack-root `stack-state.json` -> `niche` (set by Module 2C `/pick-niche`)
      3. `templates/website-template/` fallback (contractor default)

    Returns a (template_dir, niche_slug) tuple. niche_slug is None for the
    default fallback.
    """
    candidates: list[str] = []
    if cli_niche:
    ...
    for slug in candidates:
        # Accept either the exact slug or a sanitised variant (slugify spaces / underscores).
        ...
        for candidate in {slug, normalised}:
            ...
            if not (tdir / "package.json").exists():
                continue
            # Generation-failed marker: Module 2D's validator (tools/
            # validate-niche-template.py) writes GENERATION-FAILED.md into
            # the niche template directory when any hard gate fails. Stage
            # 10.1 must NOT build from a flagged template; fall back to the
            # default website-template and continue so the student gets a
            # working client build either way.
            failed_marker = tdir / "GENERATION-FAILED.md"
            if failed_marker.exists():
                print(
                    f"  WARN: niche template '{candidate}' has GENERATION-FAILED.md; "
                    f"falling back to default website-template. See "
                    f"{failed_marker.relative_to(REPO_ROOT)} for details.",
                    file=sys.stderr,
                )
                continue
            return tdir, candidate

    return DEFAULT_TEMPLATE_DIR, None


def niche_playbook_dir(niche_slug: str | None) -> Path | None:
    """Return the niche-playbook directory for the active niche, or None when
    the build is using the default website-template (which has no playbook)."""
    if not niche_slug:
        return None
...
def copy_client_photos_to_public(paths: dict[str, Path], site_dir: Path) -> dict[str, int]:
    ...
    the website-template's <img src='/work/...' /> + '/team/...' references resolve.
...
def _compose_pages_block(
    ...
    """Compose the per-page copy block consumed by the website-template page
    components. Stage 7 brand-dna-agent may set `brand_dna_in.pages.X` for any
    sub-key; this function fills missing sub-keys with sentinel-filled defaults
    so React component reads never hit `undefined`.

    The canonical shape (8 sub-keys) lives in
    `templates/website-template/src/config/brand-dna.example.js`. The validator
...
def copy_assets(client_name: str, paths: dict[str, Path], site_dir: Path, brand_dna: dict[str, Any]) -> None:
    ...
    # Founder.jsx handles 404 gracefully via onError.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Stage 10.1: clone templates/website-template, overlay brand-dna, copy assets, build, validate."
    )
