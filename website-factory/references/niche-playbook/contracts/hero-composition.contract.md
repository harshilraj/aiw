...
Prompt fragment Stage 9 (Nano Banana hero image) reads. Defines the niche's
hero photograph composition: subject placement, foreground/midground/
background, what objects anchor the shot, mood baseline.

Module 2D writes one per niche into
`templates/{niche-slug}/niche-playbook/hero-composition.md`.

A complete `hero-composition.md` MUST cover the sections below. Output is
fed into `tools/generate-hero.py` as a prompt fragment that fills the
universal scaffold at `templates/nano-banana-hero-prompt.template.md`.
...
A worked example of how the prompt fragment fills the universal scaffold.
Useful as a test fixture: feed this into `tools/generate-hero.py
