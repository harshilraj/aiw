...
```
- TrustStrip claim count: {N} (universal default: 4)
- TrustStrip badge count: {N} (from trust-signals.json `trustStripCount`)
- Process section step count: {N} (from process.json `stepCount`)
- Reviews shown on homepage: {N} (typical 4-10)
- FAQ items shown: {N} (typical 6-8)
...
Sections that must render even if data is sparse. Typical: Hero, TrustStrip,
Reviews, Founder/About, Final-CTA, Footer.
