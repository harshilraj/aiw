...
|---|---|---|
| 1 | Variable font + humanist sans pairing (Fraunces + Plus Jakarta Sans) | `templates/website-template/src/index.css` (@import), `tailwind.config.js` (fontFamily) |
...
| 3 | 1.25 / 1.333 modular type scale | `templates/website-template/tailwind.config.js` (`fontSize` extend; named `display-sm/md/lg/xl` tokens) |
...
| 9 | Scroll-linked headline reveal | `templates/website-template/src/components/ScrollRevealHeadline.jsx` |
| 10 | `prefers-reduced-motion`, no `transition: all` | `templates/website-template/src/index.css` (`@media (prefers-reduced-motion: reduce)` block); SOP 13 enforces no `transition: all` |
...
| 14 | Mobile sticky inquiry bar (no desktop) | `templates/website-template/src/components/MobileCtaBar.jsx` (already existed) |
...
| H3 | Eyebrow + Headline + Sub + One CTA above fold | `niche-playbook/hero-composition.md`; niche template's `Hero.jsx` |
...
| T# | Tactic | Where |
|---|---|---|
| T1 | Real reviews with named source, photo, date | `.claude/skills/research/SKILL.md` review extraction; niche template's `Reviews.jsx` |
| T2 | Host face shown once, prominently | `niche-playbook/photo-manifest.json` (people category); niche template's `Founder.jsx` |
